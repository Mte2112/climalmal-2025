...
VIMFD_PATH = "../../data/raw/climate_data/era5/vert_integr_moist_div_mon_era5_2004-2024.grib"
...
def load_supporting_data(malawi_shape_path, district_shape_path, lake_path, etopo_path):
    ...
    dist_shape = dist_shape[dist_shape["ID_1"] != 10].rename(columns={"ID_1": "DISTRICT"})
...
def compute_malaria_statistics(mal, dist_shape, casevar):
    ...
    malavg["geometry"] = pd.merge(malavg, dist_shape, how="left", on="DISTRICT")["geometry"]
    ...
    mal = mal.groupby('DISTRICT').apply(
    ...
    pc1_sm = atools.apply_filter_cases(pd.DataFrame({"CASES": pc1}), "CASES", filter_type='low', per=24)
    pc2_sm = atools.apply_filter_cases(pd.DataFrame({"CASES": pc2}), "CASES", filter_type='low', per=24)
    ...
    sm_pc1_cases_24 = pd.DataFrame({"DISTRICT": 1, casevar: pc1_cases_24}).groupby("DISTRICT").apply(
        lambda g: atools.apply_filter_cases(g, casevar, filter_type='low', per=24)
    ).reset_index(drop=True)["CASES_smoothed"]
    sm_pc2_cases_24 = pd.DataFrame({"DISTRICT": 1, casevar: pc2_cases_24}).groupby("DISTRICT").apply(
        lambda g: atools.apply_filter_cases(g, casevar, filter_type='low', per=24)
    ).reset_index(drop=True)["CASES_smoothed"]
    ...
    return (mal, malavg, malavgscale, nattot, cases_pivot, pc1, pc2, 
            sm_pc1_cases_24, sm_pc2_cases_24, explained_variance)
...
def process_sst_data(sst_path, y1, y2, pc1, pc2):
    ...
    tropatl_anom_nosmooth = tropatl_weighted_nosmooth.groupby("time.month") - tropatl_base_nosmooth
    ...
    tropatl_weighted_smooth = atools.area_weighted_mean(filtered_sst, (-8, 2), (-40, -10))
    ...
    return (tropatl_weighted_nosmooth, io_sst_weighted_nosmooth, 
            tropatl_anom_nosmooth, io_anom_nosmooth,
            tropatl_weighted_smooth, io_weighted_smooth,
            correlation_map_pc1, correlation_map_pc2)
...
def process_wind_data(wind_path, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth):
    ...
    wind850mb = xr.open_mfdataset(wind_path).sortby("time").sel(time=slice(str(y1), str(y2)))
    ...
    u_reg_tropatl = atools.regmap_ts_simple_fast(u_anom, tropatl_anom_nosmooth.sst).compute()
    v_reg_tropatl = atools.regmap_ts_simple_fast(v_anom, tropatl_anom_nosmooth.sst).compute()
...
def process_geopotential_data(geop_300_path, geop_850_path, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth, 
                             tropatl_weighted_smooth, tropatl_weighted_nosmooth):
    ...
    geop_300_anom = geop_300.groupby("time.month") - geop_300.groupby("time.month").mean()
    ...
    geop_850_anom = geop_850.groupby("time.month") - geop_850.groupby("time.month").mean()
    
    # regress geop on sst
    geop_300_anom = geop_300_anom.chunk({'time': -1})
    reg_geop_300_anom_tropatl = atools.regmap_ts_simple_fast(geop_300_anom.z, tropatl_anom_nosmooth.sst).compute()
    reg_geop_300_anom_io = atools.regmap_ts_simple_fast(geop_300_anom.z, io_anom_nosmooth.sst).compute()
    
    geop_850_anom = geop_850_anom.chunk({'time': -1})
    reg_geop_850_anom_tropatl = atools.regmap_ts_simple_fast(geop_850_anom.z, tropatl_anom_nosmooth.sst).compute()
    reg_geop_850_anom_io = atools.regmap_ts_simple_fast(geop_850_anom.z, io_anom_nosmooth.sst).compute()
    
    return (geop_850_height_mean, reg_geop_300_anom_tropatl, reg_geop_300_anom_io,
            reg_geop_850_anom_tropatl, reg_geop_850_anom_io)

def process_precipitation_data(precip_path, y1, y2, malawi_shape, tropatl_weighted_smooth, io_weighted_smooth, 
                              dist_shape, mal):
    """process CHIRPS precipitation data"""
    print("Processing precipitation data...")
    
    # read chirps precip
    precip_raw = xr.open_mfdataset(precip_path).sortby("time").sel(
        longitude=slice(25, 45), latitude=slice(-20, -8), time=slice(str(y1), str(y2))
    ).compute()
    ...
    precip_mal = precip_mal.rio.clip(malawi_shape_buffered.geometry.apply(mapping), malawi_shape_buffered.crs, drop=True)
    
    # get average precip
    precip_mal = atools.area_weighted_mean(precip_mal["precip"], (-90, 90), (0, 360))
    precip_mal_nov_mar = precip_mal.where(precip_mal.time.dt.month.isin([11, 12, 1, 2, 3]), drop=True)
    precip_mal_jun_sep = precip_mal.where(precip_mal.time.dt.month.isin([6, 7, 8, 9]), drop=True)
    ...
    # run regression
    precip_reg_TA = atools.regmap_ts_simple_fast(precip, tropatl_weighted_smooth).compute()
    ...
    malavg_precip = mal.reset_index()  # this should use malavg but keeping consistent with original
    malavg_precip["corr_precip"] = pd.merge(malavg_precip, dist_shape, how="left", on="DISTRICT")["corr_precip"]
...
def process_vimfd_data(vimfd_path, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth):
    ...
    reg_vimdf_TA = atools.regmap_ts_simple_fast(vimdf_anom.vimdf, tropatl_anom_nosmooth.sst).compute()
    reg_vimdf_IO = atools.regmap_ts_simple_fast(vimdf_anom.vimdf, io_anom_nosmooth.sst).compute()
    
    # spatially smooth data for plotting 
    new_lon = np.linspace(reg_vimdf_TA.longitude[0].item(), reg_vimdf_TA.longitude[-1].item(), reg_vimdf_TA.sizes["longitude"] * 4)
    new_lat = np.linspace(reg_vimdf_TA.latitude[0].item(), reg_vimdf_TA.latitude[-1].item(), reg_vimdf_TA.sizes["latitude"] * 4)
...
def process_soil_moisture_data(grace_path, grace_malawi_path, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth, 
                              sm_pc1_cases_24, sm_pc2_cases_24, casevar, dist_shape, mal):
    """process GRACE soil moisture data"""
    print("Processing soil moisture data...")
    
    # read GRACE surface soil moisture
    gracevar = "sfsm_inst"
    soil_moisture_malreg = fdata.process_nc_malawi(grace_path, y1, y2, [25, 45, -20, -8], "mean").sortby("time")
    soilm_mean_region = atools.area_weighted_mean(soil_moisture_malreg[gracevar], (-90, 90), (0, 360))
    ...
    soilm_anom = soil_moisture_malreg.groupby("time.month") - soilm_mean_region.groupby("time.month").mean()
    ...
    # set up regression
    soilm_anom = soilm_anom.chunk({'lat': -1, 'lon': -1, 'time': -1})["sfsm_inst"].rename({"lat": "latitude", "lon": "longitude"})
    # handle nans 
    ...
    # soil regressionsz
    soil_reg_tropatl = atools.regmap_ts_simple_fast(soilm_anom, tropatl_weighted_nosmooth_anom.sst).compute()
    soil_reg_io = atools.regmap_ts_simple_fast(soilm_anom, io_sst_weighted_nosmooth_anom.sst).compute()
    ...
    cutoff = 1 / per
    order = 3
    filtered_grace = atools.apply_filter_xr(soil_moisture[gracevar], per, fs, cutoff, order)
    ...
    sm_pc1_cases_24_da = xr.DataArray(sm_pc1_cases_24, dims='time', coords={'time': filtered_grace.time})
    correlation_map_soilm_pc1 = xr.corr(filtered_grace, sm_pc1_cases_24_da, dim='time').compute()
    
    # get correlation soil and pc2 malaria
    sm_pc2_cases_24_da = xr.DataArray(sm_pc2_cases_24, dims='time', coords={'time': filtered_grace.time})
    correlation_map_soilm_pc2 = xr.corr(filtered_grace, sm_pc2_cases_24_da, dim='time').compute()
    ...
    malavg_soilm["corr_soilm"] = pd.merge(malavg_soilm, dist_shape, how="left", on="DISTRICT")["corr_soilm"]
    ...
    # smoothing after eof
    sm_soilm_pc1 = pd.DataFrame({"DISTRICT": 1, casevar: pc1_soilm_mal}).groupby("DISTRICT").apply(
        lambda g: atools.apply_filter_cases(g, casevar, filter_type='low', per=36)
    ).reset_index(drop=True)["CASES_smoothed"]
    sm_soilm_pc2 = pd.DataFrame({"DISTRICT": 1, casevar: pc2_soilm_mal}).groupby("DISTRICT").apply(
        lambda g: atools.apply_filter_cases(g, casevar, filter_type='low', per=36)
    ).reset_index(drop=True)["CASES_smoothed"]
    
    return (soil_reg_tropatl_masked, soil_reg_io_masked, correlation_map_soilm_pc1,
            correlation_map_soilm_pc2, malavg_soilm, sm_soilm_pc1, sm_soilm_pc2)

def process_temperature_data(temp_path, y1, y2, malawi_shape, tropatl_anom_nosmooth, io_anom_nosmooth, 
                           dist_shape, mal, io_weighted_smooth):
    ...
    temp = temp_global.where(
        (temp_global.longitude >= 25) & (temp_global.longitude <= 45) & 
        (temp_global.latitude >= -20) & (temp_global.latitude <= -8)
    ...
    temp_mal = temp_mal.rio.clip(malawi_shape_buffered.geometry.apply(mapping), malawi_shape_buffered.crs, drop=True)
    ...
    # temp regressions
    temp_reg_tropatl = atools.regmap_ts_simple_fast(temp_anom, tropatl_weighted_nosmooth_anom.sst).compute()
    temp_reg_io = atools.regmap_ts_simple_fast(temp_anom, io_sst_weighted_nosmooth_anom.sst).compute()
    ...
    malavg_temp["corr_temp"] = pd.merge(malavg_temp, dist_shape, how="left", on="DISTRICT")["corr_temp"]
    ...
    temp_global = xr.open_dataset(temp_path).sel(time=slice(str(y1), str(y2)))
    io_sst_weighted_smooth = io_weighted_smooth  # use the smoothed version from SST processing
    ...
    return temp_reg_tropatl_masked, temp_reg_io_masked, malavg_temp, temp_IO_pos_zonal_anom
...
def process_soil_texture_data(soil_texture_path):
    ...
    colors_mapped_soiltex = {3: "#fdb9c2", 6: "#f6a077", 7: "#c49138",
                            8: "#818232", 9: "#48714f", 10: "#1e5c62", 12: "#0f3c5f"}
...
def main():
    ...
    (mal, malavg, malavgscale, nattot, cases_pivot, pc1, pc2, 
     sm_pc1_cases_24, sm_pc2_cases_24, explained_variance) = compute_malaria_statistics(mal, dist_shape, casevar)
    
    # process SST data
    (tropatl_weighted_nosmooth, io_sst_weighted_nosmooth, 
     tropatl_anom_nosmooth, io_anom_nosmooth,
     tropatl_weighted_smooth, io_weighted_smooth,
     correlation_map_pc1, correlation_map_pc2) = process_sst_data(SST_PATH, y1, y2, pc1, pc2)
    
    # process wind data
    (u, v, u_mean, v_mean, u_reg_tropatl, v_reg_tropatl, 
     u_reg_io, v_reg_io) = process_wind_data(WIND_PATH, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth)
    
    # process geopotential data
    (geop_850_height_mean, reg_geop_300_anom_tropatl, reg_geop_300_anom_io,
     reg_geop_850_anom_tropatl, reg_geop_850_anom_io) = process_geopotential_data(
        GEOP_300_PATH, GEOP_850_PATH, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth, 
        tropatl_weighted_smooth, tropatl_weighted_nosmooth)
    
    # process precipitation data
    (precip_reg_masked_TA, precip_reg_masked_IO, 
     malavg_precip) = process_precipitation_data(PRECIP_PATH, y1, y2, malawi_shape, tropatl_weighted_smooth, 
                                                io_weighted_smooth, dist_shape, mal)
    
    # process VIMFD data
    reg_vimdf_TA_smoothed, reg_vimdf_IO_smoothed = process_vimfd_data(VIMFD_PATH, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth)
    
    # process soil moisture data
    (soil_reg_tropatl_masked, soil_reg_io_masked, correlation_map_soilm_pc1,
     correlation_map_soilm_pc2, malavg_soilm, sm_soilm_pc1, 
     sm_soilm_pc2) = process_soil_moisture_data(GRACE_PATH, GRACE_MALAWI_PATH, y1, y2, tropatl_anom_nosmooth, io_anom_nosmooth,
                                               sm_pc1_cases_24, sm_pc2_cases_24, casevar, dist_shape, mal)
    
    # process temperature data
    (temp_reg_tropatl_masked, temp_reg_io_masked, malavg_temp, 
     temp_IO_pos_zonal_anom) = process_temperature_data(TEMP_PATH, y1, y2, malawi_shape, tropatl_anom_nosmooth, 
                                                       io_anom_nosmooth, dist_shape, mal, io_weighted_smooth)
    
    # process soil texture data
    gldas_resampled, colors_mapped_soiltex, soil_labels = process_soil_texture_data(SOIL_TEXTURE_PATH)
    
    # prepare all data for savingg
    data_to_save = {
        # basic parameters
        'y1': y1, 'y2': y2, 'casevar': casevar,
        
        # geographic 
        'malawi_shape': malawi_shape, 'dist_shape': dist_shape, 
        'lake_malawi': lake_malawi, 'etopo01': etopo01,
        
        # malaria 
        'mal': mal, 'malavg': malavg, 'malavgscale': malavgscale, 
        'nattot': nattot, 'cases_pivot': cases_pivot,
        'pc1': pc1, 'pc2': pc2, 'sm_pc1_cases_24': sm_pc1_cases_24, 
        'sm_pc2_cases_24': sm_pc2_cases_24, 'explained_variance': explained_variance,
        ...
        # wind 
        'u': u, 'v': v, 'u_mean': u_mean, 'v_mean': v_mean,
        'u_reg_tropatl': u_reg_tropatl, 'v_reg_tropatl': v_reg_tropatl,
        'u_reg_io': u_reg_io, 'v_reg_io': v_reg_io,
